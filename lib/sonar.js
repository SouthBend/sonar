...
const api = botBuilder(function (request, originalRequest) { // Claudia JS main function
   /* if(request.attachments){
    ...
    return restoreCtx(sender).then(function(existingCtx){ // Restore information based on that sender id to grab their state
        ...
        if(request.text === 'fire' || request.text === 'map' || request.text === 'trash' || request.text === 'other'){ // If they have already seen the welcome message, their request will be one of these
            ...
            if(state === 'other'){ // If they chose the 'other' section, you are now talking about knowledge articles instead of data like fire/trash
                return persistCtx(sender, state).then(function(result){ // You want to add that they are no longer in the welcome state, but have rather been asked for more information
                    const Q2 = new fbTemplate.Text('Okay, I can look in the 311 Knowledge Center for you. What would you like me to search for?'); // see ClaudiaJS fbTemplate. Just a nice way of using FB messenger features
                    return Q2
                        .get();
                });
            }
            else if (state === 'trash'){
              return persistCtx(sender, state).then(function(result){
                const Q2 = new fbTemplate.Text('I can find out when trash pick up is.  What address are you interested in?')
                return Q2
                  .get();
              });
            }
              ...
              return persistCtx(sender, state).then(function(result){
                const Q2 = new fbTemplate.Text('I can find the nearest fire hydrant for you.  What address are you interested in?')
                return Q2
                  .get();
              });
            }
        ...
        if(state === 'fire' || state === 'map' || state === 'trash' || state === 'other' || request.text === 'about'){// This is now the second time the program is run. Since the person has selected an option, and is now responding with a location
